...
local function tile_group(cls, wa, orientation, fact, group)
    ...
    for c = group.first,group.last do
        ...
        then
            ...
            if top then
                geom[height] = geom[height] - (2 + gap_factor) * useless_gap + 0
                geom[y] = geom[y] + useless_gap
            else
                geom[height] = geom[height] - (1 + gap_factor) * useless_gap + 0
            end

            if left then
                geom[width] = geom[width] - (2 + gap_factor) * useless_gap + 0
                geom[x] = geom[x] + useless_gap
            else
                geom[width] = geom[width] - (1 + gap_factor) * useless_gap + 0
            end
        end
        -- End of useless gap.
